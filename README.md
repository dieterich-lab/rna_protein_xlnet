...
- `splitpos`: If it is set to `None` fine-tuning is carried out with the percentage splits given with `splitratio`. If a splits position is given, we expect at least three different splits on which we do cross validation by:
